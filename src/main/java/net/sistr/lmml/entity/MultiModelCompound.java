...
public class MultiModelCompound implements IHasMultiModel {
    ...
    private final ArmorsHolder<TextureHolder> armorsTexHolder = new ArmorsHolder<>();
    ...
    private String getName(Item item) {
        //クライアント限定
        if (entity.world.isRemote && item instanceof ArmorItem) {
            return ((ArmorItem) item).getArmorMaterial().getName().toLowerCase();
        }
    ...
    private void updateArmorPart(Part part, String armorName, float damagePercent) {
        ...
        ArmorPartModelData.Builder dataBuilder = ArmorPartModelData.Builder.newInstance();
    ...
    @Override
    public boolean isAllowChangeTexture(@Nullable Entity changer, TextureHolder textureHolder, Layer layer, Part part) {
        return true;
    }
