...
public class ModelRenderer {
    ...
    //前バージョンでは頭がおかしかったのかObjectの可変長引数にしていたが、どう考えても無駄である
    public static void setParam(MatrixStack matrixStack, IVertexBuilder buffer, int packedLight, int packedOverlay,
                                float red, float green, float blue, float alpha) {
        ModelRenderer.matrixStack = matrixStack;
        ModelRenderer.buffer = buffer;
        ...
        for (ModelBoxBase modelBoxBase : this.cubeList) {
            for (ModelBoxBase.TexturedQuad quad : modelBoxBase.quadList) {
                ...
                for (int i = 0; i < 4; ++i) {
                    ...
                    bufferIn.addVertex(vector4f.getX(), vector4f.getY(), vector4f.getZ(),
        ...
        switch (rotatePriority) {
            case RotXYZ:
                if (rotateAngleZ != 0.0F) {
                    matrixStack.rotate(Vector3f.ZP.rotation(rotateAngleZ));
                }
                if (rotateAngleY != 0.0F) {
                    matrixStack.rotate(Vector3f.YP.rotation(rotateAngleY));
                }
                if (rotateAngleX != 0.0F) {
                    matrixStack.rotate(Vector3f.XP.rotation(rotateAngleX));
                }
                break;
            case RotXZY:
                if (rotateAngleY != 0.0F) {
