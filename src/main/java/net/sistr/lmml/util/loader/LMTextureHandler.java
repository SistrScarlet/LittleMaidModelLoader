...
public class LMTextureHandler implements ILMFileLoaderHandler {
     ...
     * キャッシュファイル名
     */
    private final String cacheFileName = "cache_texturepack.json";
    ...
    public boolean isLoadHandler(String path, Path filePath) {
        ...
        if (path != null && path.endsWith(".png")) {
            ...
            return !getTextureName(path).isEmpty();
        }
        return false;
    }
    ...
    public void loadHandler(String path, Path filePath, InputStream inputstream) {
        ...
        //マイクラ側にリソースを読み込ませる
        if (filePath != null) {
            ResourceWrapper.addTexturePath(path, filePath.toString());
        }
        ...
        if (colorIndex != -1) {
            ...
            LittleMaidModelLoader.LOGGER.error(String.format("LMTextureHandler-ColorIndexException : %s", path));
        }
    }
